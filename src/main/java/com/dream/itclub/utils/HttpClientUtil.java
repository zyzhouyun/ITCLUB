...
/**
     * 发送短信方法
     *
     * @param smsMob  接受短信的手机号
     * @param smsText 发送的短信内容
     * @return
     *//*

    public int sendMessages(String smsMob, String smsText) {
        HttpClientUtil client = HttpClientUtil.getInstance();//*实例化短信接口对象
        //UTF发送
        int result = client.sendMsgUtf8("J饮多佐", "d41d8cd98f00b204e980", smsText, smsMob);//执行UTF的短信发送
        if (result > 0) {//判断发送结果
            System.out.println("UTF8成功发送条数==" + result);//输出成功及发送的条数
        } else {
            System.out.println(client.getErrorMsg(result));//输出异常信息
        }
        return result;
    }
